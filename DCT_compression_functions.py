...
def DCTBlocks(blocks):
    ...
    w=blocks.shape[2];
    h=blocks.shape[3];
    output_blocks=np.zeros((yLen,xLen,h,w,3));
    for y in range(yLen):
        for x in range(xLen):
            d = np.zeros((h,w,3))
            for i in range(3):
...
def BlockQuantization(blocks,quantization_ratio):
    ...
    Q3 = np.dstack([QY*quantization_ratio]*3)
    Q3=Q3*((11-w)/3)
    ...
    for y in range(yLen):
        for x in range(xLen):
            for i in range(3):
                ...
                rbCount[y*xLen*3+x*3+i]=HuffmanCodingOneArray(zz[y*xLen*3+x*3+i])
...
def SavingQuantizedDctBlocks(blocks,bitBits=3,runBits=1):
    ...
    for y in range(yLen):
        for x in range(xLen):
            for i in range(3):
                codeNew,DC=RunLength(zigZag[y*xLen*3+x*3+i],DC,hfm )
                code+=codeNew
